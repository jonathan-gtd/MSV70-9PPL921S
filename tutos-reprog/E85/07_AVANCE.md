...
OBJECTIF E85 SI DÉMARRAGE DIFFICILE (+1° à +2° sur colonnes froides) :
TCO (°C) →  -30.0  -20.3   -9.8    0.0   +9.8  +30.0  +60.0  +90.0
...
(autres lignes identiques au stock)
```
...
**Rôle :** Même rôle qu'opm_1 mais pour le mode papillonné (Valvetronic désactivé au démarrage). Si le N52 démarre en mode GD, c'est cette table qui s'applique. Modifier en même temps qu'opm_1 si la correction est nécessaire.
...
OBJECTIF E85 SI DÉMARRAGE DIFFICILE (+1° sur colonnes froides) :
Appliquer le même incrément que pour opm_1 sur les colonnes TCO ≤ 0°C.
```
